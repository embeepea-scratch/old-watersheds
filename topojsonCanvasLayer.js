...
function walkarc(arcs, i, map, ctx, first) {
...
function ring_bbox(bbox, arcs, ring) {
    ring.forEach(function(i) {
        var arc = arcs[(i >= 0) ? i : ones_complement(i)];
...
function polygon_bbox(bbox, arcs, rings) {
    // expand an existing bbox by the coordinates in each of an array of rings
    // modifies bbox in-place
    rings.forEach(function(ring) {
        ring_bbox(bbox,arcs,ring);
    });
}
function geom_bbox(geom, arcs) {
    var bbox = [[null,null],[null,null]];
    if (geom.type === "Polygon") {
        polygon_bbox(bbox, arcs, geom.arcs);
    } else if (geom.type === "MultiPolygon") {
        // geom.arcs is an array of polygons as above
        geom.arcs.forEach(function(polygon) {
            polygon_bbox(bbox, arcs, polygon);
        });
    }
    return bbox;
}
